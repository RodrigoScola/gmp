...
export const RenderChatMesages = (props) => {
    ...
    const d = useRefetch(`http://localhost:3000/api/chat/snuffy/?page=${currentPage}`);
    const lmessage = useRefetch(`http://localhost:3000/api/chat/snuffy/?first=${currentPage}`);
    useUpdateEffect(() => {
        if (!lmessage?.data ||
    ...
    useUpdateEffect(() => {
        if (!d?.data)
            return;
        if (d.data.messages[0].id == allChat.messages[0].id)
            return;
