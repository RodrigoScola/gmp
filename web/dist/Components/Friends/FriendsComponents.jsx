...
export const FriendCardOpen = ({ friend, isOpen, ...props }) => {
    ...
    return (<div className={`px-4 bg-gray-800  rounded-lg py-2 text-white   z-10 ${props.className}`} {...props}>
               <div className="rounded-t-xl py-2 pl-1 rounded-b-md">
                    <div className="flex  flex-row items-center gap-4">
                         <FriendAvatar boxSize={`2em`} badgeprops={{
            boxSize: `1em`,
        }} friend={friend}/>

                         <Link className="capitalize text-xl font-bold text-white" href={`/user/${friend.username}`}>
                              {friend.username}
                         </Link>
                    </div>
               </div>
            ...
            handleFriend.sendInvite("Rock Paper Scissors");
                    ...
                    <div className="pt-1">
                         <form className="gap-1 inline-flex w-full" onSubmit={handleNewMessage}>
                              <div className="flex flex-col w-full">
                                   <p className="font-whitney mb-2 font-semibold">
                                        Message
                                   </p>
                                   <input onChange={(e) => setMessage(e.target.value)} className="search-input w-full" value={message} placeholder={`Message @${friend.username}`}/>
                              </div>
                         </form>
                    </div>
               </div>
...
export const FriendCard = (props) => {
    ...
    return (<>
               <Popover isOpen={isOpen} placement="left" onOpen={onOpen} onClose={onClose}>
                    <PopoverTrigger>
                    ...
                    <PopoverContent className="text-black noselect p-0 mr-4 left-20 border-none z-10">
                         <FriendCardOpen isOpen={isOpen} friend={props.friend}/>
                    </PopoverContent>
               </Popover>
          </>);
};
...
export const InviteFriendComponent = (friendId) => {
